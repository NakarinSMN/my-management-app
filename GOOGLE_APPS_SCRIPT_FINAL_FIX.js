// Google Apps Script สำหรับจัดการข้อมูลลูกค้า (แก้ไข timezone และวันที่)
...
function addCustomer(data) {
  try {
    ...
    const newRow = [
      ...
      data.customerName || '',
      data.phone || '',
      dateObj || '', // ใช้ Date object หรือค่าว่าง
      data.note || '',
      '', // userId
      365 // day
    ];
...
function updateCustomer(data) {
  try {
    ...
    // หาแถวที่มีทะเบียนรถเดิม (normalize ทั้งสองฝั่ง)
    let targetRow = -1;
    ...
    const updateRow = [
      ...
      data.customerName || '',
      data.phone || '',
      dateObj || allData[targetRow-1][5], // ใช้ Date object ใหม่หรือเก็บเดิม
      data.note || '',
